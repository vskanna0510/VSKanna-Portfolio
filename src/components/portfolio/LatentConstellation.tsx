...
const CLUSTERS: Cluster[] = [
  {
    id: "vision",
    label: "Vision",
    project: "Deepfake detection — 94% accuracy",
    color: "#4F46E5",
    cx: 0.22,
    cy: 0.28,
  },
  {
    id: "nlp",
    label: "Text",
    project: "BERT fine-tuning on custom data",
    color: "#F97316",
    cx: 0.78,
    cy: 0.26,
  },
  {
    id: "audio",
    label: "Audio",
    project: "Audio spectrogram model",
    color: "#8B7BD8",
    cx: 0.22,
    cy: 0.78,
  },
  {
    id: "multi",
    label: "Combined",
    project: "Text + image + audio — 87% accuracy",
    color: "#10B981",
    cx: 0.78,
    cy: 0.76,
  },
...
export function LatentConstellation({
  ...
  const fusingRef = useRef<{
    cluster: number | null;
    t0: number;
    phase: "spiral" | "burst" | "ring";
  } | null>(null);
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      ctx.fillStyle = grad;
      ctx.beginPath();
      ctx.arc(coreX, coreY, 100 + coreFlash * 60, 0, Math.PI * 2);
      ctx.fill();

      ctx.fillStyle = `rgba(255,255,255,${0.92 + coreFlash * 0.08})`;
      ctx.beginPath();
      ctx.arc(coreX, coreY, coreR + coreFlash * 14, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.arc(coreX, coreY, coreR + 12 + pulse * 6, 0, Math.PI * 2);
      ctx.stroke();
  ...
  return (
    <div className="relative">
      <div
        ...
        className="relative h-[360px] w-full overflow-hidden rounded-2xl border border-white/10 sm:h-[440px]"
      >
