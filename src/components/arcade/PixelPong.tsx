...
export function PixelPong({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    let py = H / 2 - 40, ay = H / 2 - 40;
    let bx = W / 2, by = H / 2, vx = 4 * (Math.random() < 0.5 ? -1 : 1), vy = 3 * (Math.random() < 0.5 ? -1 : 1);
    let sp = 0, sa = 0;
    ...
    const reset = (winner: "p" | "a") => {
      if (winner === "p") sp++; else sa++;
      setScore({ p: sp, a: sa });
      bx = W / 2; by = H / 2;
      vx = (winner === "p" ? 4 : -4); vy = 3 * (Math.random() < 0.5 ? -1 : 1);
      if (sp >= 5 || sa >= 5) {
    ...
    const draw = () => {
      if (!stateRef.current.paused && !over) {
        ...
        bx += vx; by += vy;
        if (by < 6 || by > H - 6) { vy *= -1; blip(220, 0.04); }
        // Player paddle collision
        if (bx < 28 && by > py && by < py + 80) { vx = Math.abs(vx) * 1.05; vy += (by - (py + 40)) * 0.08; blip(440); }
        if (bx > W - 28 && by > ay && by < ay + 80) { vx = -Math.abs(vx) * 1.05; vy += (by - (ay + 40)) * 0.08; blip(330); }
        if (bx < 0) { blip(150, 0.2, "sawtooth"); reset("a"); }
        if (bx > W) { blip(660, 0.2, "sawtooth"); reset("p"); }
      }
      ...
      ctx.fillStyle = "#4F46E5"; ctx.fillRect(16, py, 8, 80);
      ctx.fillStyle = "#F97316"; ctx.fillRect(W - 24, ay, 8, 80);
      ctx.fillStyle = "#1E1B1B"; ctx.beginPath(); ctx.arc(bx, by, 6, 0, Math.PI * 2); ctx.fill();

      raf = requestAnimationFrame(draw);
    };

    const onKey = (e: KeyboardEvent, down: boolean) => { keys[e.key] = down; if (["ArrowUp","ArrowDown"].includes(e.key)) e.preventDefault(); };
    const kd = (e: KeyboardEvent) => onKey(e, true);
    const ku = (e: KeyboardEvent) => onKey(e, false);
    const mm = (e: MouseEvent) => { const r = canvas.getBoundingClientRect(); mouseY = ((e.clientY - r.top) / r.height) * H; };
  ...
  const restart = () => { setScore({ p: 0, a: 0 }); setOver(null); /* re-mount via key */ };
