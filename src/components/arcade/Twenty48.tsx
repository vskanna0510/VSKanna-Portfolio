...
export function Twenty48({ onClose }: { onClose: () => void }) {
  ...
  const [best, setBest] = useState(
    () => Number(typeof window !== "undefined" ? localStorage.getItem("arc-2048-best") : 0) || 0,
  );
  ...
  const move = useCallback(
    (dir: 0 | 1 | 2 | 3) => {
      ...
      if (moved) {
        ...
        if (ns > best) {
          setBest(ns);
          localStorage.setItem("arc-2048-best", String(ns));
        }
        if (!muted && gained > 0) {
          try {
            ...
            o.frequency.value = 400 + Math.log2(gained) * 80;
            o.type = "sine";
            gN.gain.value = 0.06;
            o.connect(gN);
