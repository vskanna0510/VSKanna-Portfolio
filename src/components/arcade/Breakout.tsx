...
export function Breakout({ onClose }: { onClose: () => void }) {
  ...
  const [best, setBest] = useState(() => Number(typeof window !== "undefined" ? localStorage.getItem("arc-breakout-best") : 0) || 0);
  ...
  useEffect(() => {
    ...
    let raf = 0;
    let bx = W / 2, by = H - 40, vx = 3.2, vy = -3.2;
    let px = W / 2 - 50, pw = 100;
    ...
    const tick = () => {
      if (!paused) {
        ...
        bx += vx; by += vy;
        if (bx < 6 || bx > W - 6) { vx *= -1; beep(440); }
        if (by < 6) { vy *= -1; beep(520); }
        ...
        if (by > H) {
          if (s > best) { setBest(s); localStorage.setItem("arc-breakout-best", String(s)); }
          bx = W / 2; by = H - 40; vx = 3.2; vy = -3.2; s = 0; setScore(0);
          bricks.forEach(b => (b.alive = true));
        }
        for (const b of bricks) {
          if (!b.alive) continue;
          if (bx > b.x && bx < b.x + BRICK_W && by > b.y && by < b.y + BRICK_H) {
            b.alive = false; vy *= -1; s += 10; setScore(s); beep(880);
            break;
          }
        }
      }
  ...
  return (
    <GameShell
      title="Breakout" controls="← → or mouse"
      score={score} best={best}
      onClose={onClose}
      onRestart={() => { setScore(0); setRestartKey(k => k + 1); }}
      paused={paused} setPaused={setPaused}
      muted={muted} setMuted={setMuted}
    >
      <div className="relative">
        <canvas ref={ref} width={W} height={H} className="w-full rounded-xl border border-white/10" />
